...
const mapTwoToThe = (input) => {
  // Solution code here...
  return input.map((integer) => 2 ** integer);
}
...
const evenOdd = (input) => {
  // Solution code here...
  return input.map((integer) => isNaN(integer) ? 'N/A' : integer % 2 === 0 ? 'even' : 'odd');
};
...
const extractStats = (stats) => {
  ...
  return stats.map((stat) => move = {name: stat.stat.name, total: stat.effort + stat.baseStat});
};
