...
const getStatName = (input, minBaseStat) => {
  // Solution code here...
  return getBaseStatGreaterThan(input, minBaseStat).map((stat) => stat.stat.name);
};
...
const evenOddNumericValues = (input) => {
  // Solution code here...
  return input.filter((maybeANumber) => typeof(maybeANumber) === 'number').map((number) => number % 2 === 0 ? 'even' : 'odd');
};
